...
const ArtistPageThirdHeader = (props) => {
  const getTime = (time) => {
  ...
  return (
    <Box px={4} pt={2}>
      <Typography sx={{ fontSize: "30px", color: "#fff", fontWeight: 600 }}>
        Popular
      </Typography>
      <List sx={{ color: "White", pt: 2 }}>
        {props.data &&
          props.data.data.artist.discography.topTracks.items.map(
            (content, index) => (
              <ListItem
                sx={{
              ...
              >
                ...
                >
                  ...
                  <ListItemAvatar sx={{ pl: 0 }}>
                    <Avatar
                      variant="square"
                      src={content.track.album.coverArt.sources[0].url}
                    />
                  </ListItemAvatar>
                  <ListItemText
                    sx={{ pl: 0, flexGrow: 0 }}
...
export default ArtistPageThirdHeader;
